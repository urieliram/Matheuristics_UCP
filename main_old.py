...
name_list   = [] ## start-up  cost

T = len(md.data['system']['time_keys'])  ## time periods

## To get the data from the generators
for item, gen in md.elements("generator", generator_type="thermal", in_service=True): 
    
    Piecewise.append(gen["p_cost"]["values"])
    lista=[]
    for i in range(1,len(Piecewise[g])+1):
        lista.append(i)
    L[g+1] = lista
    ...
    #print(Startup)
    lista=[]
    for i in range(1,len(Startup[g])+1):
        lista.append(i)  
    S[g+1] = lista
...
model    = UC.solve(g,T,L,S,Pmax,Pmin,UT,DT,De,Re,u_0,U,D,SU,SD,RU,RD,pc_0,mpc,
                   Pb,C,Cs,Tmin,name_list,fixShedu,relax,ambiente)
